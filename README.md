...
### The header elements
Title and logo are above the navigation. The title says EUROPE and the logo finalize the title with letters Q, U I and Z in a box. Depending on the media query the header gets slightly bigger (font-size: 105%). The content in the header is consitent through all three html. pages. It cotains two links and a styling for the active site. The header is fixed to the top.

<img src="images_README/navigation_view.PNG" alt="image shows header">

### The main elements
The main elements differ in each html content and structurewise and consists most features and buttons. It is positioned via top-margin: 100px. In the following all features of the three pages are described.
...
| Feature | Description  | images |
| --- |------------- | ------------- |
| Input filed for user's name | At the end of the game the user can save his score. For that an input field is provided to put in his name. By using trim() a string with whitespace stripped from the beginning and end of the string is returned.| <img src="images_README/end_game_over_saveHighscore.PNG" alt="image shows a user who saves his score"> |
...
<img src="" alt="image shows the end display of the game">
...
- Font Awesome icons were used, for GitHub and Linked icons in the footer
...
## Wireframes
Wireframes were developed for mobile, tablet, and desktop sizes by using Balsamiq. Due to ongoing disucssions with the demand site the wireframes differ form the end results. The end results contains visual improvements and the chapter "about" is not generated. Information about the creator can be easily find by opening the GitHUb or Linkedin profile of the creator.
...
- due to testing and small bugs wihtin gitpod (there was also a version update from basic to pro thorugh out the project) code was transfered in between.


### Bugs
background missing on deployed version.


| Bug | Description  | images (optional) |
| --- |------------- | ------------- |
| wrong path used | absolute instead of relative path used. Deployed verison showed error 404 when the button redirects the user back to the index.html | <img src="" alt="image shows"> |
| invalid constant | The validaotr shoed that an old variable was still in the code | <img src="" alt="image shows"> |
| missing semicolon | Due to a missing semicolon the function was not working | <img src="" alt="image shows"> |
| Wrong definition of a class element | Symbol # was used instead of . in the stlye.css file. As a consequence of the element answer was not linked with the correct css style. The responsive effect was not working. | <img src="" alt="image shows"> |
...
|7| visibility | ... | 
|8| social icons | Code Institude | project runaway
|9| slider | https://www.w3schools.com/howto/howto_css_switch.asp


ToDo

- check credits code incl. number in code css, js
- correct background on deployed version
- check images in readme.md
- functional test once agian
- check typos, describe bugs in more detail
- add media links
- add padding-top to h1
